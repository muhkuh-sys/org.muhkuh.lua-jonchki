--- A repository driver for the nexus artifact server.
-- The repository module provides an abstraction to a number of different
-- repositories. The real work is done by drivers. This is the nexus
-- driver providing the functions for the Sonatype Nexus server.
...
-- This module requires "lxp" to parse the XML response from the Lucene search engine.
repository_driver.tLxp = require("lxp")
...
function repository_driver:get_search_chunk(strArtifact, uiFrom, uiCount)
	local fOk = nil
	local strData = nil
...
function repository_driver.parseSearch_StartElement(tParser, strName)
...
function repository_driver.parseSearch_EndElement(tParser, strName)
	local aLxpAttr = tParser:getcallbacks().userdata
...
function repository_driver.parseSearch_CharacterData(tParser, strData)
	local aLxpAttr = tParser:getcallbacks().userdata
...
function repository_driver:parse_search_chunk(strChunk, atArtifactList)
	local fOk = nil
	local tResult = nil
...
function repository_driver:search(strArtifact)
	local fOk = true
	local tResult = nil
	local atArtifacts = {}
	local uiOffset = 0
...
function repository_driver:get_resolve_data(atArtifact, strClassifier, strExtension)
	local fOk = nil
	local strData = nil
	-- Collect the results in this array.
	local t = {}
...
-- @param tParser The parser object.
-- @param strName The name of the new element.
function repository_driver.parseResolve_StartElement(tParser, strName)
	local aLxpAttr = tParser:getcallbacks().userdata
...
-- @param tParser The parser object.
-- @param strName The name of the closed element.
function repository_driver.parseResolve_EndElement(tParser, strName)
	local aLxpAttr = tParser:getcallbacks().userdata
...
-- @param tParser The parser object.
-- @param strData The character data.
function repository_driver.parseResolve_CharacterData(tParser, strData)
	local aLxpAttr = tParser:getcallbacks().userdata
...
function repository_driver:parse_resolve_data(strData)
	local fOk = nil
	local tResult = nil
...
function repository_driver:download_artifact(atArtifact, strClassifier, strExtension)
	local fOk = nil
	local tResult = nil
...
function repository_driver:download(atArtifact, strClassifier, strExtension)
	local fOk = true
	local tResult = nil
	-- Collect the results in this array.
	local t = {}
